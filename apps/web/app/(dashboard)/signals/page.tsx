...
function hrefFor({ severity, period, date }: { severity?: Severity; period: "today" | "yesterday" | "5d" | "all" | "date"; date?: string }) {
...
export default async function SignalsPage({ searchParams }: { searchParams: { severity?: string; view?: string; period?: string; date?: string } }) {
  ...
  const signalOptions = period === "all" ? undefined : period === "5d" ? { signalDate: taipeiDate(), days: 5 } : { signalDate: period === "yesterday" ? taipeiDate(1) : selectedDate };
  ...
  return <div>
    ...
    <div className="space-y-3 border-b border-border px-6 py-3">
      <div className="flex flex-wrap items-center gap-3"><span className="section-label">日期</span><Filter href={hrefFor({ severity, period: "today" })} active={period === "today"}>今日</Filter><Filter href={hrefFor({ severity, period: "yesterday" })} active={period === "yesterday"}>昨天</Filter><Filter href={hrefFor({ severity, period: "5d" })} active={period === "5d"}>近 5 天</Filter><Filter href={hrefFor({ severity, period: "all" })} active={period === "all"}>全部</Filter><form action="/signals" className="flex items-center gap-1"><input type="hidden" name="period" value="date" />{severity && <input type="hidden" name="severity" value={severity} />}<input name="date" type="date" defaultValue={selectedDate} className="rounded border border-border-light bg-panel px-2 py-1 font-mono text-xs text-text" /><button className="rounded border border-border px-2 py-1 text-xs text-text-dim hover:text-text">查看日期</button></form><span className="ml-auto font-mono text-[11px] text-text-faint">{signals.length} signals</span></div>
      <div className="flex flex-wrap items-center gap-3"><span className="section-label">Severity</span><Filter href={hrefFor({ severity: undefined, period, date: period === "date" ? selectedDate : undefined })} active={!severity}>All</Filter><Filter href={hrefFor({ severity: "p1", period, date: period === "date" ? selectedDate : undefined })} active={severity === "p1"}>P1</Filter><Filter href={hrefFor({ severity: "p2", period, date: period === "date" ? selectedDate : undefined })} active={severity === "p2"}>P2</Filter><Filter href={hrefFor({ severity: "observe", period, date: period === "date" ? selectedDate : undefined })} active={severity === "observe"}>Observe</Filter></div>
    </div>
